...
export const getServerStatus = async (req: Request, res: Response): Promise<void> => {
	...
	client.setex(`${req.baseUrl}${req.url}`, 5, JSON.stringify(response), (err, rep) => {
		if (err || rep !== "OK") console.error(`Error when setting server status to Redis: ${err}`)
		else return res.status(200).json(response)
	})
}

export const getPlayerStatus = async (req: Request, res: Response): Promise<void> => {
	...
	client.setex(`${req.baseUrl}${req.url}`, 5, JSON.stringify(response), (err, rep) => {
		if (err || rep !== "OK") console.error(`Error when setting player status to Redis: ${err}`)
		else return res.status(200).json(response)
	})
}
